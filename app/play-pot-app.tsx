...
export default function PlayPotApp() {
  ...
  // Prototype A keeps the bottom-anchored list scrolled to the dock.
  useEffect(() => {
    const list = listRef.current;
    if (list) list.scrollTop = list.scrollHeight;
  }, [state?.revision, authState]);
  ...
  return (
    <main className="app-shell proto proto-a">
      {compactHeader}
      <div className="pmain" ref={listRef}>
        <div className="pa-list">
          {recentBlock}
          {byDue.length ? (
            [...byDue].reverse().map((family) => (
              <CompactFamilyRow
                key={family.id}
                family={family}
                now={now}
                disabled={busy}
                onEdit={editFor(family)}
                action={
                  <button
                    type="button"
                    className="cr-out"
      ...
      <section className="pdock" aria-label="New family">
        {dockSteppers}
        {visualInput}
        {enterButton}
      </section>
