...
export default function PlayPotApp() {
  ...
  return (
    <main className="app-shell proto proto-b">
      {compactHeader}
      <div className="pmain" ref={listRef}>
        {mode === "in" ? (
          <section className="pb-panel" aria-label="New family">
            {nextDueFamily && nextDueTimer ? (
              <button
                type="button"
                className={`pb-next ${nextDueTimer.overdue ? "is-due" : ""}`}
                onClick={() => setMode("out")}
              >
                <span>NEXT DUE</span>
                <strong>
          ...
          <section className="pb-panel" aria-label="Families inside">
            {recentBlock}
            {byDue.length ? (
              <div className="pb-grid">
                {byDue.map((family) => {
                  const timer = shortTimer(family, now);
                  return (
                    <article key={family.id} className={`bt ${timer.overdue ? "bt-due" : ""}`}>
                      <button
                        type="button"
                        className="bt-out"
                        disabled={busy}
                        aria-label={`Check ${familyLabel(family)} out`}
                        onClick={(event) => openOut(family, event.currentTarget)}
                      >
                        <span className="bt-top">
                          <strong>{familyLabel(family)}</strong>
                          <span className="bt-timer">
                            <b>{timer.big}</b> {timer.small}
                          </span>
                        </span>
                        <span className="bt-visual">{family.visual || "No visual"}</span>
                        <span className="bt-meta">
                          {familyPax(family)} PAX · {family.adults}A {family.children}C
                        </span>
                        <span className="bt-cta">OUT</span>
                      </button>
                      <button
                        type="button"
                        className="bt-edit"
                        aria-label={`Edit ${familyLabel(family)}`}
                        onClick={() => setEditSheetId(family.id)}
                      >
                        Edit
                      </button>
                    </article>
      ...
      <nav className="pb-tabs" aria-label="Mode">
        <button
          type="button"
          className={`pb-tab ${mode === "in" ? "is-active" : ""}`}
        ...
        <button
          type="button"
          className={`pb-tab ${mode === "out" ? "is-active" : ""}`}
          aria-pressed={mode === "out"}
          onClick={() => setMode("out")}
        >
          CHECK OUT · {activeFamilies.length}
          {overdueCount ? <span className="pb-badge">{overdueCount} DUE</span> : null}
        </button>
      </nav>
      {editSheet}
